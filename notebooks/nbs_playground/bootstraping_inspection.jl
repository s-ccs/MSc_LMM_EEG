...
# ╔═╡ a6c509d6-3134-4057-bf8f-1b50154ab1f2
begin
	using DataFrames
	using MixedModels
	using Statistics
...
# ╔═╡ d6c50a30-f092-486d-b838-946b263fb4c4


# ╔═╡ c3e228b5-e7aa-4aa8-b79a-f41c8242d7ba
d1 = DataFrame(
	subj = ["A","B","A","B","A","B","A","B"],
	item = ["1", "1", "1", "1", "2", "2", "2", "2"],
	cond = ["I", "I", "I", "I", "II","II","II","II"],
	#y = [3,3,3,3,3,3,3,3] # => fails cause response constant
	#y = [3,3,3,3,7,7,7,7] # => sometimes matrix not positive definite? 
	#y = [3,3,3,3,10,10,10,10] 
	y = [5,10,5,10,5,10,5,10] 
)

# ╔═╡ c3dff5bc-5146-4285-8d85-dbaa2301b205
d = DataFrame(
	subj = ["A","B","A","B"],
	item = ["1", "1","1", "1"],
	cond = ["I", "I", "II", "II"],
...
# ╔═╡ 91509b60-425d-4ddc-b68c-54bd4820e9ee
data = repeat(d1, 1)

# ╔═╡ 51ee19ca-a1c3-4d5e-a5ca-4f9c21e0994b
f = @formula y ~ 1 + cond + (1|subj)

# ╔═╡ 7eb11c1b-d2dc-45e1-8506-f26a773f9fc9
contrasts = Dict(:item => DummyCoding())

# ╔═╡ e1f3257d-ce41-4372-87b1-e1e4967f5640
# ╠═╡ show_logs = false
m = fit(MixedModel, f, data, contrasts=contrasts)

# ╔═╡ 068038c0-1e02-4bba-baec-9c4752e9f264
vcov(m)

# ╔═╡ 0d076539-b063-4b64-84cd-239ed4de3712
begin
	arr = repeat([5,10],1)
	mean(arr), std(arr), var(arr)
end

# ╔═╡ 05bed8ce-f476-43e3-914b-35e46d866789
arr

# ╔═╡ dca416c6-3bae-4bd8-8ab2-99623b7c5e6a
begin
	const rng = MersenneTwister(1234321);
	samp = parametricbootstrap(rng, 10_000, m);
	df = DataFrame(samp.allpars)
end

# ╔═╡ 80cc571a-1ee7-4c7a-a9eb-1f71ff5b2dec
begin
	σres = filter(df) do row # create a thunk that operates on rows
	    row.type == "σ" && row.group == "residual" # our filtering rule
	end
	
	plot(x = σres.value, Geom.density, Guide.xlabel("Parametric bootstrap estimates of σᵣ"))
end

# ╔═╡ 9280b39c-6859-48a7-989b-94b6444d63d2
begin
	β1 = filter(df) do row # create a thunk that operates on rows
		    row.type == "β" && row.names == "(Intercept)" # our filtering rule
		end
	plot(x = β1.value, Geom.density, Guide.xlabel("Parametric bootstrap estimates of β₁"))
end

# ╔═╡ 8012cf11-993a-4092-94f7-a81bb88d63dd


# ╔═╡ 87e55307-a937-40d2-a4b3-2cd5f2562398
begin
	β2 = filter(df) do row # create a thunk that operates on rows
		    row.type == "β" && row.names == "cond: II" # our filtering rule
		end
	plot(x = β2.value, Geom.density, Guide.xlabel("Parametric bootstrap estimates of β₂"))
end

# ╔═╡ 63cc6682-f1dd-45f6-9f4b-e149569f5a36


# ╔═╡ dc8e0920-531c-4226-8d69-ed30f1e1cb59
begin
	σsubj = filter(df) do row # create a thunk that operates on rows
	    row.type == "σ" && row.group == "subj" # our filtering rule
	end
	plot(x = σsubj.value, Geom.density,
	    Guide.xlabel("Parametric bootstrap estimates of σ subj"))
end

# ╔═╡ 6cf47e2b-b066-40bd-9fac-0ba15544e77a
plot(x = σsubj.value, Geom.histogram,
    Guide.xlabel("Parametric bootstrap estimates of σ₁"))

# ╔═╡ 0b7bdc55-dd38-41e8-96fe-5ff55b762e5a
begin
	σitem = filter(df) do row # create a thunk that operates on rows
	    row.type == "σ" && row.group == "item" # our filtering rule
	end
	plot(x = σitem.value, Geom.density,
	    Guide.xlabel("Parametric bootstrap estimates of σ item"))
end

# ╔═╡ 6c40b116-0887-4192-9b2d-c221913fe774
plot(x = σitem.value, Geom.histogram,
    Guide.xlabel("Parametric bootstrap estimates of σ₁"))
