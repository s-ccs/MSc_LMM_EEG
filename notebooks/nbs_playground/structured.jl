...
# ╔═╡ 2531193b-6784-47ea-8ebb-0666aa9685de
begin
	using Parameters
	using DataFrames
	using MixedModels
...
# ╔═╡ 842325d3-3bd0-4f09-88ee-388db9ae2c8f
using SignalAnalysis

# ╔═╡ 1b9ae09f-8232-4652-8fff-e2f9172c2ec5
using PlutoUI, FFTW

# ╔═╡ 09bdb50a-e3e6-4d7f-9672-2cca0453b8f7


# ╔═╡ c889d0c4-34b3-48e1-b80e-0aaf0600af6f
plotly()

# ╔═╡ 8529010b-2c18-463f-aeeb-9493b9f2befd
begin
...
# ╔═╡ 70d68acc-523c-4faa-82cc-262348529d92
struct ExpDesign
	n_subj :: Int
	n_item :: Int
	n_trial :: Int
	subj_btwn :: Union{Dict, Nothing}
	item_btwn :: Union{Dict, Nothing}
	both_win :: Union{Dict, Nothing}
end

# ╔═╡ ce2b6265-907b-48ff-bb11-a7ed7563518b
begin
	dims(expdesign::ExpDesign) = expdesign.n_subj * expdesign.n_item * expdesign.n_trial
	
	generate(expdesign::ExpDesign) = DataFrame(
...
# ╔═╡ abc7627a-416a-4032-bd94-bfa09aae1ca1


# ╔═╡ 90d37400-4e4e-45cf-a552-1e7f16bd3411
struct UnfoldSim
	rng::MersenneTwister
	f :: FormulaTerm
	expdesign :: ExpDesign
...
# ╔═╡ 4554861c-6917-454b-b851-5755d1278b0f
begin	
	function simulate(ufS::UnfoldSim)
	    # simulate epoch data
		epoch_dat = simulate_epoch_dat(ufS)
	
		# convolve events with erps
		#s = epoch_dat * evt_onsets
		slen = 500
		#evt_onsets = [[1];[300]]
		evt_onsets = ufS.evt_onsets
		evt_onsets_oh = hcat([Int.([i in e for i in 1:slen]) for e in eachrow(evt_onsets)]...)'
		eeg_dat = hcat([conv(epoch_dat[i,:], evt_onsets_oh[i,:]) for i in 1:size(evt_onsets_oh,1)]...)'[:,1:floor(Int64, slen)]
	...
	function simulate_epoch_dat(ufS::UnfoldSim)
		# fit mixed model to experiment design and dummy data
		m = MixedModels.fit(MixedModels.MixedModel, ufS.f, generate(ufS.expdesign))
	
		# empty epoch data
		epoch_dat = zeros(Int(ufS.epoch_len), dims(ufS.expdesign))
		...
		for t in 1:ufS.epoch_len
			...
			MixedModelsSim.update!(m, [MixedModelsSim.create_re((b .* (r)./ ufS.σ_lmm)...) for r in ufS.re]...)
	
	        # simulate with new parameters
			simulate!(deepcopy(ufS.rng), m, β = b .* [ufS.β...], σ = ufS.σ_lmm)
...
# ╔═╡ 200f10d8-f275-4bf9-bd43-b13c3dddea79
ufs = UnfoldSim(
	MersenneTwister(1),
	@formula(dv~1+stimType+(1+stimType|subj)+(1|item)),
	ExpDesign(2,2,1, nothing, Dict("stimType" => ["I", "II"]), nothing),
	100,
	Hanning(),
	1,
	[[2,0],[1]],
...
# ╔═╡ 68f1330e-c73e-4a4e-abbc-92604cd158fd
d = simulate_epoch_dat(ufs)

# ╔═╡ 7c9c387c-8ca9-4048-bd55-f3da6edcc0e1
plot(d')

# ╔═╡ 05afb3ce-b306-4ce0-b939-bcfd21e0e835
d2 = simulate(ufs)

# ╔═╡ 91d60509-750e-49d9-b251-44edd82424d0
plot(d2')

# ╔═╡ 1596fd1b-c82e-46e0-b639-29b8c6fa48e1
md"""
# Noise
"""

# ╔═╡ dcb50898-a768-471d-8a71-041e45aa13db


# ╔═╡ 58c72337-9c41-4beb-8e85-557139ec485d
let
	ts = 0.001
	tmax = 10
	t = 0:ts:tmax
	n = length(t)
	signal = randn(MersenneTwister(3),n)
	#signal = rand(MersenneTwister(3), PinkGaussian(n, 1.0))
	#signal = rand(MersenneTwister(4), RedGaussian(n, 1.0))

	F = fft(signal) |> fftshift
	freqs = fftfreq(length(t), 1/ts) |> fftshift

	plot(freqs[freqs.>=0], amp2db.(abs.(F[n÷2+1:n])), xscale=:log10, xlims=(100,10000))

	plot(signal)
end

# ╔═╡ eea9cec2-3029-418f-9cd7-5c4145189f56
let
	ts = 0.001
	tmax = 10
	t = 0:ts:tmax
	n = length(t)
	signal = sum(reshape(randn(MersenneTwister(3),2*n), (2,:)), dims=1)
...
# ╔═╡ 82a87e1b-5f38-44c4-8bd1-987c1eca7cc7
sum.([[1 2]; [3 4]; [5 6]], dim=1)

# ╔═╡ 8cabc38e-47ab-468b-b59f-b8075d339cdd
A = [[1 2]; [3 4]; [5 6]]

# ╔═╡ 4d4c2ba4-11eb-479c-b2af-2a0f630f753b
sum(A, dims=1)
