...
# ╔═╡ 8999b1f2-6078-43db-98ee-55a0a45b0af5
import Pkg

# ╔═╡ 5d9a7e35-cf32-4e61-bae7-c485605992fe
using StatsModels, MixedModels, DataFrames, CategoricalArrays

# ╔═╡ 147a4159-7c57-46c6-a863-bf17328dadc9
using Unfold

# ╔═╡ a1735873-9327-43bc-a039-efea0ecd5570
using CSV

# ╔═╡ b07f3f55-082c-4d01-b29f-85c29e73e38b
using DSP

# ╔═╡ c5a6b042-9aab-4385-97f7-f7359a061f6c
using Plots

# ╔═╡ 9dad1c05-9b9e-4df9-bb69-cc87f3bc2b1e
include("/Users/luis/git/Unfold.jl/test/test_utilities.jl")

# ╔═╡ 3824053a-a2d3-429e-b7e6-d9e26a244ffe


# ╔═╡ eb4cbf3b-d8b9-498e-a10e-a9fbfbf734a6
md"----"

# ╔═╡ baff403b-24ca-46bd-acf8-be07b211383e
data, evts = loadtestdata("testCase3",dataPath = "/Users/luis/git/Unfold.jl/test/data/");

# ╔═╡ a1ae9208-74ce-48ad-ac21-358622b405b2
evts_new = filter(row->(row.subject <= 10) && (0 <= row.condA + row.condB <= 2) && (row.stimulus in [2,3,4,5,6,7,8,9,10]), evts)

# ╔═╡ b765cf28-f9a2-4df0-a001-ac8106eabb5e
groups = groupby(evts_new, :subject)

# ╔═╡ e6663fb6-2df7-44ec-8d3a-2c5594078586
begin
	new_groups = []
	for gdf in groups
		arr = zeros(10)
		...
		for (i, row) in enumerate(eachrow(gdf))
			if arr[row.stimulus] == 0
				arr[row.stimulus] = 1
			else
				push!(d, i)
			end
		end
		push!(new_groups, gdf[Not(d), :])		
	end
end

# ╔═╡ 641aa9c2-59d1-462b-acf4-91e38673290f
evts_N = vcat(new_groups...)

# ╔═╡ 4f954999-11d1-44dc-90c8-ce34f30f1e5e
rem = filter(row->(row.subject <= 10) && ((row.condA + row.condB == 2) || (row.condA + row.condB == 0)), evts).latency

# ╔═╡ beaedb1d-b45e-43dd-ad2c-81c4be83d33e
data_new = deepcopy(data)

# ╔═╡ 453b70ce-1614-4b15-a4cc-fdcdf723e193
data_new[rem] .= 0

# ╔═╡ 3f9ad34b-e4af-4eeb-80fb-4e6d489af950
data_ = data_new[1:last(evts_new.latency) + 5]

# ╔═╡ 21721eb7-af74-4807-8667-6ad53b567bd7
plot(data_[1:300])

# ╔═╡ 3ef9a45d-8cae-4aba-bc8c-019dd929e16b


# ╔═╡ cf0f2b76-fa67-4382-b081-1222cf706658
md"----"

# ╔═╡ 918b2d8e-a85c-477a-a3ef-6d183230c79d
data_noise = data_ .+ 0.01*randn(size(data_))

# ╔═╡ 99c81ab7-112c-49c2-956d-5e5f30bb57c0
evts_new

# ╔═╡ 9f67f746-320c-4d2d-aa8a-7c35769167d7
evts_N

# ╔═╡ d6add64c-a7bb-4710-ae7e-d4f26b616ad2
data_epoch, times = Unfold.epoch(data=data_noise, tbl=evts_N, τ=(-0.1,.1), sfreq=50);

# ╔═╡ 4e0e6728-22a1-494a-8086-ec8e11491f16
begin
	p = plot(title="Epochs")
		for i in 1:size(evts_N, 1)
			plot!(times, data_epoch[:, :, i]')#, label=l1[i], c=c[i], ls=ls[i])
		end
		p
end

# ╔═╡ 7c753d91-8580-4b4c-8fde-ee7acd4d6dd4
evts_new.subject  = categorical(Array(evts_new.subject))

# ╔═╡ 76243fcc-00e4-430c-bc19-382b23fa8eb4
#basisfunction = firbasis(τ=(-0.1,.2),sfreq=50,name="stimulus")

# ╔═╡ d2436744-9e60-409a-b98a-6313dcc21282
f  = @formula 0~0+condA+condB + (1|subject);

# ╔═╡ 5e2821be-8f52-4f32-b258-44812248e77b
bfDict = Dict(Any=>(f, times))

# ╔═╡ 5b7439e3-c4f2-4a20-996e-e50b655de3e3
data_N = convert(Array{Float64, 3}, data_epoch)

# ╔═╡ 82a4c349-7dda-42a2-93da-67f6007fd586
m = fit(UnfoldModel, bfDict, evts_N, data_N)

# ╔═╡ b258789d-39be-4146-b82e-549c329ec140
res = coeftable(m)

# ╔═╡ 012f6c8f-1d3c-4dd6-9848-5ff8df574703
condA = filter(row->row.coefname=="condA", res).estimate

# ╔═╡ d8fe6d7a-819d-451a-812b-1eb0e542baef
condB = filter(row->row.coefname=="condB", res).estimate

# ╔═╡ 3d934929-c4cb-4a24-a24b-e47fad6ed82e
plot([condA, condB])

# ╔═╡ b3f33934-e6a2-44af-96c0-18223fe0c7b3
intr = filter(row->row.coefname=="(Intercept)", res).estimate
...
[[deps.MixedModels]]
deps = ["Arrow", "DataAPI", "Distributions", "GLM", "JSON3", "LazyArtifacts", "LinearAlgebra", "Markdown", "NLopt", "PooledArrays", "ProgressMeter", "Random", "SparseArrays", "StaticArrays", "Statistics", "StatsAPI", "StatsBase", "StatsFuns", "StatsModels", "StructTypes", "Tables"]
git-tree-sha1 = "01ef21968d7c25d08df50f091370550862709426"
