...
# ╔═╡ 03419b90-7da2-4a3b-9f0b-e4ab8c6e3f40
begin 
	using DataFrames
...
# ╔═╡ ea4f983b-dede-4568-aa57-40d606d8ba99
using Distributions

# ╔═╡ f696cc7c-2321-11ed-3ca3-3dfe55338c9d
md"# Unstructured"

# ╔═╡ ebed8714-cda5-4e5d-8691-754975d569bf
begin
	n_subj = 2
	n_item = 2
	...
	# with overlaps possible
	#event_onsets = [sort(sample(MersenneTwister(i),1:sig_len, n_evts, replace = false)) for i in 1:d]

	# without overlaps possible
	ar = collect(1:sig_len÷epoch_len)
	evto  =[[sample(MersenneTwister(i*k),i*epoch_len-epoch_len+1:i*epoch_len, 1, replace = false)[1] for i in ar[Bool.(ar .% 2)]] for k in 1:n_item:d]

	event_onsets = [evto[i][k:2:sig_len÷(2*epoch_len)] for i in 1:n_subj for k in 1:n_item]

	noise_level = 0
end;

# ╔═╡ e091dedb-f3f6-4303-b03c-a397a18a27ab
begin
	ex = sort!(DataFrame(
...
# ╔═╡ 6c2c3301-a495-4813-bf5c-9a367937d6bf
begin
	...
	# empty epoch data
	epoch_data = zeros(Int(epoch_len), n_subj*n_item)

	# generate basis function
	b2s = 1//3
	bf_pos = 2
	basis = zeros(epoch_len, 1)
	b_len = floor(Int, epoch_len * b2s)
	s = b_len * (bf_pos-1)
	e = s + b_len
	basis[s+1:e] .= DSP.hanning(b_len)

	# residual variance for lmm
	σ_lmm = 0.0001
		
	# iterate over each timepoint
	for t in 1:epoch_len
	...
	evt_onsets_oh = hcat([Int.([i in e for i in 1:sig_len]) for e in event_onsets]...)'
	
	# convolve onsets with epoch data
	eeg_dat = hcat([conv(epoch_data'[i,:], evt_onsets_oh[i,:]) for i in 1:size(evt_onsets_oh,1)]...)'[:,1:floor(Int64, sig_len)]
	# TODO: Fic convolution for multiple evt onsets
	
	# create noise
	noise = reshape(randn(MersenneTwister(1), length(eeg_dat)), size(eeg_dat))

	# add noise to data
	eeg_dat = eeg_dat + noise_level .* noise
end

# ╔═╡ 0d4c792f-3174-444c-a4f7-feba0cd586b0
begin
	l = ex.subj .* ex.item .* " (" .* ex.stimType .* ")";
	
	cs = ColorScheme(distinguishable_colors(20, [RGB(1,1,1), RGB(0,0,0)], dropseed=true))[2:end]
end;

# ╔═╡ 20910520-2dd7-44ad-90a9-9594a2d806bf
begin
	ls = [:solid, :dash, :dot, :dashdot, :dashdotdot][1:n_item]
	cl = cs[1:n_subj]
end;

# ╔═╡ 558ca63b-fb0f-4dff-a070-5deeabb464bb
begin
	with_theme(linewidth=3,Lines=(; cycle=[:linestyle, :color])) do
		...
		for (i, line) ∈ enumerate(eachcol(epoch_data))
...
# ╔═╡ d81cc9d3-c9ad-4e2f-a57c-c5b24c863f39
let
	rng = MersenneTwister(2)
	deepcopy(rng)
end

# ╔═╡ 7713596f-5bcc-4f4f-a860-c2d16a010a08
begin
	with_theme(linewidth=2,Lines=(; cycle=[:linestyle, :color])) do
...
# ╔═╡ aa0b11bb-979c-4185-a4cf-cce612345dad
x = 1

# ╔═╡ fcd9c705-82d1-4ecb-a74b-cf9a2c5b5ab6
test_data.dv = epoch_data[50,:]

# ╔═╡ f546dff5-aba5-4a45-8894-295c54276d0b
test_data

# ╔═╡ e9650e82-753f-47da-98ac-3ff89d5f2324
m1 = fit(MixedModel, f, test_data, contrasts=contrasts)
