...
# ╔═╡ b4a0940e-08c5-11ed-1002-cf34dbbb193c
begin
	# Imports
...
# ╔═╡ e559ee6b-59ad-4157-b32b-f585f4042026
pink_noise = PinkGaussian(100, 3.0)

# ╔═╡ aa4d3482-7719-4e98-9d8b-38e14934cb7e
lines(rand(MersenneTwister(1), pink_noise))

# ╔═╡ 0adf4609-dc87-49a7-beb8-ac95d2056d4b
abstract type Noise end

# ╔═╡ 4378d280-25b3-4046-9361-52a97db4bbab
abstract type BasisFunction end

# ╔═╡ 541a67d9-9162-475c-9fc4-9ed579b1f1c2
@with_kw struct Hanning <: BasisFunction
	func = DSP.hanning
end

# ╔═╡ f36a9c5a-b1a9-4246-a160-7e3cbad37bdb
@with_kw struct Rect <: BasisFunction
	func = DSP.rect
end

# ╔═╡ f5825843-bec0-4454-9f3f-670fc9323fff
...
# ╔═╡ 364891a9-b54d-4142-b199-b09b9c6bd8bb
function offset(signal, len)
	return [repeat([0], len); signal]
end

# ╔═╡ ab558e58-43d2-4aff-8f09-25871490b810
@bind t PlutoUI.Slider(1:100, show_value=true)

# ╔═╡ 7839f688-1fb3-4927-a9d7-a5030b67a814
let
	bf = Hanning() 
	signal = zeros(t,1)
	ba = gen_basis(bf, size(signal, 1), 1//3, 2)
	lines(vec(ba))
end

# ╔═╡ 1d523a91-f8d5-4666-a304-bab74abb24f9


# ╔═╡ 850b0e5f-013b-442b-b905-84a353726ad1
begin
	lines(ba)
	lines!(vec(z))
	lines!(vec(z0))
...
# ╔═╡ 06265b30-844b-4e7c-9944-f6eb32680be6
z0

# ╔═╡ 3772c3cb-ed84-46d8-b416-c0226ce863cb
s

# ╔═╡ 0c7d5bc9-b149-4865-ab57-ebd181178195
a = [1,2,3,4,5]

# ╔═╡ 659b642a-aefe-4d81-bb3c-d089f35e9880
a[1:end]

# ╔═╡ 8f80ae4e-c2ab-442a-a64c-50c9992cf40f
offset(a, Int(size(z, 1) / 3))

# ╔═╡ 6ad6ad82-3477-475e-8a00-03a32dadde08
function simulate(ufS::UnfoldSim, basis::BasisFunction, f, betas::Vector{Int64}, re::Vector{Vector{Int64}}, σ_lmm::Int64)

	# fit mixed model to experiment design and dummy data
	m = MixedModels.fit(MixedModel, f, ufS.expdesign)

	
end

# ╔═╡ 86c172f6-87ef-447f-9202-b2f8f37a8e1a
begin
	rng = MersenneTwister(1)
	rng_copy = deepcopy(rng)
end

# ╔═╡ 33107c1b-d52b-49d5-8b79-0fb606328bf5
function simulate_erps(epoch_len, n_subj, n_cond, n_trial, basis_func, re, β, σ_lmm)
	# empty epoch data
	epoch_dat = zeros(Int(epoch_len), n_subj * n_cond * n_trial)

	# generate
	basis = gen_basis(basis_func)
		
	# iterate over each timepoint
	for t in 1:epoch_len
		b = basis[t]
		
		MixedModelsSim.update!(m,[create_re((b .* (r)./ σ_lmm)...) for r in re]...)

		simulate!(deepcopy(rng_copy), m, β = b .* [β...], σ = σ_lmm)
...
# ╔═╡ f9e49ecd-354e-458d-8de6-09ad0f9999ca
function simulate(ufS)
	# simulate erps
	erps = simulate_erps(ufS)
...
# ╔═╡ 68de8526-5df2-4901-9491-f75491808a06
deepcopy(rng_copy)
