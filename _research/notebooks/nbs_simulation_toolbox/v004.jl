...
# ╔═╡ 1df5c71d-c183-4210-865b-00b076fa52d2
md"""
#### Changes:
...
# ╔═╡ 5c1152e6-b816-4568-8d24-01c36d72a9ed
...
# ╔═╡ 83120762-888e-4cc3-8bdc-443b2d1b27d0
struct Component
	basis
	formula
	contrasts
...
begin
	...
	basis1 = padarray(hanning(25), (-25, 200), 0);
	basis2 = padarray(-hanning(50), (-30, 170), 0);
	basis3 = padarray(hanning(50), (-65, 135), 0);
	basis4 = padarray(hanning(150), (-90, 10), 0);
	...
	components = [custom]#, n100, p200, p300]
...
function simulate_erps(rng, design, components)
	...
	for component in components
		...
		# unpacking fields
		(; basis, formula, contrasts, β, re, σ_res) = component
...
# ╠═╡ show_logs = false
# simulate erps
erps = simulate_erps(rng, design, components);
...
"""
function simulate(rng, simulation, components)
...
# simulate eeg
eeg, onsets = simulate(rng, simulation, components);

# ╔═╡ 190c08e9-50b3-4610-9290-88bbf900a61c
onsets

# ╔═╡ a3493e3e-48fe-498d-8705-68b6026d9301
plot(eeg, title="EEG", label=permutedims(hcat(l1)))

# ╔═╡ 85e0703b-f0ee-4d63-85dc-4fb4ef87bb65
erps

# ╔═╡ 9d58ae4a-2d41-407c-842e-bd3c052c8311
ft = rand(MersenneTwister(1), 140:160, (10,6))

# ╔═╡ 4df8180c-efe0-4cf9-ba0f-e232f4f43b3b
et = [0, repeat([250], 10-1)...]

# ╔═╡ 8adb1735-2341-41c6-8eea-836f6d52caa3
tt = ft .+ et

# ╔═╡ f42a80f9-0d2d-4fa6-bcb2-b9659041f5d5
at = accumulate(+, tt, dims=1)

# ╔═╡ a251892c-1f4e-47a1-8b00-69d9e283a4e3
at[CartesianIndex(2,1)]

# ╔═╡ b7d0b134-66df-432d-9081-3bf064ec3d8d
z = hcat([[Int.(i in col) for i in 1:4000] for col in eachcol(at)]...)

# ╔═╡ 6315d609-73b4-483f-a426-7609918db7df
[(i,s) for i in 1:n_item for s in 1:n_subj]

# ╔═╡ 67145289-a9bf-4cd4-b4fa-5a124511417b
let
	fixationlen = 150 # number of average fixation samples 
	std_fixationlen = 10 # deviation from average fixation samples
	epochlen = 250 # number of samples per epoch
	min_fixationlen = fixationlen - std_fixationlen
	max_fixationlen = fixationlen + std_fixationlen
	upperbound = (epochlen + max_fixationlen) * n_item

	
	fixationlens = rand(MersenneTwister(1), min_fixationlen:max_fixationlen, (n_item, n_subj))
...
# ╔═╡ 16ae37a9-9847-4713-8439-30e8f58c1483
upperbound

# ╔═╡ cbc272e9-63cf-4eb2-96ac-fb424163f556
zeros(5) + [1,2,3,4,5]

# ╔═╡ 227a896e-a301-44e6-a847-33f59456b090
CartesianIndex(1,2)

# ╔═╡ 35817595-2ec5-40eb-8be6-79d3c4df43bc
let
	fixationlen = 150 # number of average fixation samples 
	std_fixationlen = 10 # deviation from average fixation samples
	epochlen = 250 # number of samples per epoch
	...
	fixationlens = rand(MersenneTwister(1), min_fixationlen:max_fixationlen, n_item)
	epochlens = [0, repeat([epochlen], n_item-1)...]
	onsets = accumulate(+, (fixationlens .+ epochlens))

	onsets_1hot = [Int.(i in onsets) for i in 1:upperbound]
	plot(erps[:,1])
	eeg = conv(erps[:,1], onsets_1hot)
	plot(eeg)
	vline!([onsets])
	onsets
end
...
# ╔═╡ 38b8135d-dfc5-4a15-adc3-10bf8649f31c
let
	...
	cond_unfold = results.estimate

	plot(times, cond_unfold)
end

# ╔═╡ 6dd961b8-5e4b-4466-b063-8ff3cac4cfa1
evts
...
# ╔═╡ 3710fb5a-1e66-4bb1-ba47-198362741de6
plot(condA)
...
begin
	condA_ = hcat(condA_unfold...)
	condB_ = hcat(condB_unfold...)
	x = maximum.(eachcol(condA_))
	y = maximum.(eachcol(condB_))
	t = OneSampleTTest(x, y)
end

# ╔═╡ 20fb03a9-fc4e-46c8-bc24-3b8ace1cf1c3
md"#### pvalue: $(pvalue(t))"
